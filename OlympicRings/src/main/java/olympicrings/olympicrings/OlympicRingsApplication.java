package olympicrings.olympicrings;
...
public class OlympicRingsApplication extends Application {
   ...
   *
   * @param centerX the x coordinate of the center of the ring
   * @param centerY the y coordinate of the center of the ring
   ...
   */
  private static Arc generateRingSegment(int centerX, int centerY, int start,
      int length,
      Color colour) {
    Arc arc = new Arc(centerX, centerY, OlympicRingsApplication.RING_RADIUS,
   ...
   *
   * @return int[][] the 2d array of ring centers [1....][x,y]
   */
  private static int[][] generateRingPositions() {
    ...
    int[][] ringPositions = new int[5][2];

    //ROW 1
    //ring 0
    ringPositions[0][0] =
        RING_RADIUS + ((WINDOW_WIDTH - (3 * RING_DIAMETER) - (2 * RING_SPACING)) / 2);
    ringPositions[0][1] = FIRST_ROW_Y;

    //ring 1
    ringPositions[1][0] = ringPositions[0][0] + RING_DIAMETER + RING_SPACING;
    ringPositions[1][1] = FIRST_ROW_Y;

    //ring 2
    ringPositions[2][0] = ringPositions[1][0] + RING_DIAMETER + RING_SPACING;
    ringPositions[2][1] = FIRST_ROW_Y;

    //ROW 2

    //ring 3
    ringPositions[3][0] = (ringPositions[0][0] + ringPositions[1][0]) / 2;
    ringPositions[3][1] = SECOND_ROW_Y;

    //ring 4
    ringPositions[4][0] = (ringPositions[1][0] + ringPositions[2][0]) / 2;
    ringPositions[4][1] = SECOND_ROW_Y;
  ...
  public void start(Stage stage) {
    ...
    int[][] ringPositions = generateRingPositions();
    ...
    pane.getChildren()
        .add(generateRingSegment(ringPositions[2][0], ringPositions[2][1], -70, 290,
            Color.RED));

    //draw first green
    pane.getChildren()
        .add(generateRingSegment(ringPositions[4][0], ringPositions[4][1], 40, 100,
            Color.GREEN));

    //draw first black segment
    pane.getChildren()
        .add(generateRingSegment(ringPositions[1][0], ringPositions[1][1], -70, 290,
            Color.BLACK));

    //draw first gold segment goes in-front of black segment
    pane.getChildren()
        .add(generateRingSegment(ringPositions[3][0], ringPositions[3][1], 40, 100,
            Color.GOLD));

    //draw first blue segment goes in-front of gold segment
    pane.getChildren()
        .add(generateRingSegment(ringPositions[0][0], ringPositions[0][1], 0, 360,
            Color.BLUE));

    //close the gold ring going in-front of blue
    pane.getChildren()
        .add(generateRingSegment(ringPositions[3][0], ringPositions[3][1], 40, -260,
            Color.GOLD));

    //close the black ring, going in-front of gold
    pane.getChildren()
        .add(generateRingSegment(ringPositions[1][0], ringPositions[1][1], -70, -70,
            Color.BLACK));

    //close the green ring, going in-front of black
    pane.getChildren()
        .add(generateRingSegment(ringPositions[4][0], ringPositions[4][1], 40, -260,
            Color.GREEN));

    //close the black ring, going in-front of gold
    pane.getChildren()
        .add(generateRingSegment(ringPositions[2][0], ringPositions[2][1], -70, -70,
            Color.RED));
